...
1. 代码实际行为 > 2. `AGENTS.md` > 3. `PAI.md` > 4. `README.md` > 5. `CLAUDE.md`

## 项目快照

- 项目名：`Mini Claude Code`
- 定位：面向商业使用的 Java Agent CLI 产品
- 能力：ReAct / Plan-and-Execute / Multi-Agent 三套执行模式；记忆与上下文工程、RAG 代码理解、多模型、联网、MCP、Chrome DevTools 浏览器自动化、HITL 安全层、Skill、TUI、LSP 诊断、Side-Git 快照回滚、Prompt 分层、后台任务 + Runtime API、图片输入、微信通道
- `PAI.md` 是 Mini Claude Code 的项目级记忆文件：启动时自动注入 system prompt，适合团队共享的长期稳定规则；个人/会变化的经验继续用 `/save` 长期记忆。
...
讯飞星辰 MaaS provider 名为 `xfyun`，默认 Base URL 为 `https://maas-api.cn-huabei-1.xf-yun.com/v2`。`model` 必须使用服务管控页展示的 `modelId`；公开模型名 / Hugging Face 仓库名不一定可直接调用。微调模型用 `/config provider xfyun --lora-id <resourceId>` 配置服务卡片上的 resourceId，Mini Claude Code 会作为 HTTP header `lora_id` 发出。`xfyun` 当前按 MaaS 文档走纯对话请求，不向上游发送 Mini Claude Code 内置工具列表。
...
以下未交付：容器/VM 沙箱 / MCP OAuth + sampling + server 自动重启
